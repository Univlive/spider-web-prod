...
function FooterComponent({ data, theme: t, selected, onClick }: ComponentProps) {
  return (
    <div onClick={onClick} style={{ background: t.text, padding: "48px 40px 24px", cursor: "pointer", outline: selected ? `3px solid ${t.accent}` : "none", outlineOffset: -3 }}>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}><div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)" }}>© 2025 {data.name || "Apex Institute"}. All rights reserved.</div><div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)" }}>Powered by PREPAREKARO.IN</div></div>
    </div>
  );
}
