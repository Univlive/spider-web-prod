...
export default function AssignAndScheduleDialog({
  ...
  const [globalConfig, setGlobalConfig] = useState<BatchConfig>(() =>
    defaultConfig(String(Math.min(2, initialTest?.attemptsAllowed ?? 2)), initialTest?.type === "from_dpp")
  );
  ...
  const reset = () => {
    ...
    setPerBatch(false);
    setGlobalConfig(defaultConfig(String(Math.min(2, initialTest?.attemptsAllowed ?? 2)), initialTest?.type === "from_dpp"));
    setPerBatchConfigs({});
    setSaving(false);
    setCopied(null);
  };
      ...
      <div className="space-y-3">
        ...
        {cfg.accessType === "open" && (
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-3 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">1 attempt · Auto-expires in 1 week · No code required</p>
          </div>
        )}
        ...
        {cfg.accessType !== "open" && (
          <div className="space-y-1">
            ...
            <Input
              ...
              onChange={(e) => update("attemptsAllowed", String(Math.min(2, Math.max(1, Number(e.target.value)))))}
            />
          </div>
        )}
              ...
              <div className="max-h-72 space-y-1 overflow-y-auto rounded-lg border p-2">
                {allTests.map((t) => (
                  <button
                    ...
                    onClick={() => {
                      setSelectedTest(t);
                      setGlobalConfig(defaultConfig(String(Math.min(2, t.attemptsAllowed ?? 2)), t.type === "from_dpp"));
                    }}
