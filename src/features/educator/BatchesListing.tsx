      ...
      () => {
        ...
        const fetchAllBatches = async () => {
          ...
          for (const bDoc of branchSnap.docs) {
            const courseSnap = await getDocs(collection(db, "educators", educatorId, "branches", bDoc.id, "courses"));
            for (const cDoc of courseSnap.docs) {
              const batchSnap = await getDocs(collection(db, "educators", educatorId, "branches", bDoc.id, "courses", cDoc.id, "batches"));
    ...
    <div className="p-6 max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <Card className="bg-muted/30 border-none shadow-none">
        <CardContent className="p-4 flex flex-wrap items-center gap-4">
          ...
          {branchFilter !== "All" && (
            <Button variant="ghost" size="sm" onClick={() => updateBranchFilter("All")} className="text-xs h-8">
              Reset Filters
            </Button>
          )}
        </CardContent>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredBatches.map((batch) => {
            const occupancy = Math.round(((batch.studentCount || 0) / (batch.capacity || 50)) * 100);
            const isFull = occupancy >= 90;
            const isWarning = occupancy >= 75 && occupancy < 90;

            return (
              <Card key={batch.id} className="group border-border/50 overflow-hidden">
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between">
                    <div className="p-2 bg-primary/5 rounded-xl text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-colors duration-500">
                      <Users className="h-5 w-5" />
                    </div>
                    <Badge variant="outline" className="text-[10px] font-bold uppercase tracking-wider py-0 px-2 h-5 border-muted-foreground/20">
                      {branches.find(b => b.id === batch.branchId)?.name || "Main"}
                    </Badge>
                  </div>
                  <CardTitle className="mt-4 text-xl group-hover:text-primary transition-colors">{batch.name}</CardTitle>
                ...
                <CardContent className="space-y-6 pt-4">
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    <Button 
                      ...
                      onClick={() => navigate(`/educator/students?branch=${branches.find(br => br.id === batch.branchId)?.name}&batch=${batch.name}`)}
                    >
                      View Students
                      <ChevronRight className="ml-2 h-3 w-3 group-hover/btn:translate-x-1 transition-transform" />
                    </Button>
                  </div>
                </CardContent>
              </Card>
      ...
      {!isLoading && filteredBatches.length > 0 && (
        <div className="flex items-center justify-between pt-4 border-t border-border/50">
          <p className="text-sm text-muted-foreground font-medium">
            Showing <span className="text-foreground font-bold">{filteredBatches.length}</span> active batches
          </p>
        </div>
      )}
    </div>
