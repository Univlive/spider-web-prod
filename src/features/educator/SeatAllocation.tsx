...
type SeatPool = { planId: string; planName: string; totalSeats: number; availableSeats: number; allocatedSeats: number };
type Branch = { id: string; name: string };
type Course = { id: string; branchId: string; name: string };
type Batch = { id: string; branchId: string; courseId: string; name: string; planId?: string; seatLimit: number; usedSeats: number; poolAllocatedSeats?: number };
...
export default function SeatAllocation() {
  ...
  useEffect(() => {
    ...
    getDocs(collection(db, "plans")).then((snap) =>
      setPlans(snap.docs.filter((d) => d.data().isActive).map((d) => ({ id: d.id, ...(d.data() as Omit<Plan, "id">) })))
    );
    ...
    return () => { unsubPools(); unsubBranches(); };
  }, [educatorId]);
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      for (const branch of allBranches) {
        const cSnap = await getDocs(collection(db, "educators", educatorId, "branches", branch.id, "courses"));
        for (const course of cSnap.docs) {
          const bSnap = await getDocs(
            collection(db, "educators", educatorId, "branches", branch.id, "courses", course.id, "batches")
          );
      ...
      <Card className="max-w-xl">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            {selectedBatch?.planId && (
              <p className="text-xs text-amber-600">
                Batch is locked to <b>{plans.find((p) => p.id === selectedBatch.planId)?.name || selectedBatch.planId}</b> plan.
              </p>
            )}
          </div>
          ...
          <div className="space-y-1">
            ...
            >
              <SelectTrigger><SelectValue placeholder="Select plan pool" /></SelectTrigger>
              <SelectContent>
                {pools.map((pool) => (
                  <SelectItem key={pool.planId} value={pool.planId} disabled={pool.availableSeats === 0}>
                    {plans.find((p) => p.id === pool.planId)?.name || pool.planId} — {pool.availableSeats} available
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-1">
            ...
            />
            {selectedPlanId && (
              <p className="text-xs text-muted-foreground">{availablePoolSeats} seats available in pool</p>
            )}
          </div>
                ...
                {allocatedBatches.map((batch) => {
                  ...
                  return (
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <span className={batch.usedSeats >= batch.seatLimit && batch.seatLimit > 0 ? "text-destructive font-medium" : ""}>
                      ...
                      <TableCell>
                        <Button
                          ...
                          title={returnable <= 0 ? "All seats are occupied" : `Return ${returnable} seats to pool`}
                        >
