...
function validateSlug(slug: string) {
  ...
  if (slug.length < 3 || slug.length > 40)
    throw new Error("Subdomain slug must be 3–40 characters.");
  if (!/^[a-z0-9][a-z0-9-]*[a-z0-9]$/.test(slug)) {
    throw new Error(
      "Slug can contain only lowercase letters, numbers and hyphens (no leading/trailing hyphen)."
    );
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const oldSlug0 = String(
      eduSnap0.data()?.tenantSlug || userSnap0.data()?.tenantSlug || user.profile?.tenantSlug || ""
    ).trim();

    if (!oldSlug0)
      return res.status(400).json({ error: "Your account is missing the current tenant slug." });
    if (oldSlug0 === newSlug)
      return res.json({ ok: true, oldSlug: oldSlug0, newSlug, studentsUpdated: 0 });
    ...
    if (newTenantSnap0.exists) {
      ...
      if (owner && owner !== uid)
        return res.status(409).json({ error: "This subdomain slug is already taken." });
    }
    ...
    await db.runTransaction(async (tx) => {
      ...
      const oldSlug = String(
        eduSnap.data()?.tenantSlug || userSnap.data()?.tenantSlug || user.profile?.tenantSlug || ""
      ).trim();
      ...
      tx.set(
        eduRef,
        { tenantSlug: newSlug, updatedAt: FieldValue.serverTimestamp() },
        { merge: true }
      );
      tx.set(
        userRef,
